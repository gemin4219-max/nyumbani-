...
const KARIAKOO_FEATURES = ['Brand New', 'Used', 'Wholesale', 'Delivery', 'Warranty'];

export default function ManageKariakooScreen() {
  ...
  const fetchItems = async () => {
    setLoading(true);
    const { data } = await supabase.from('kariakoo_items').select('*').order('created_at', { ascending: false });
  ...
  const handleDelete = async (id: string) => {
    Alert.alert('Delete Item', 'Are you sure you want to delete this item forever?', [
      ...
      { text: 'Delete', style: 'destructive', onPress: async () => {
        await supabase.from('kariakoo_items').delete().eq('id', id);
        fetchItems();
      }}
    ]);
  };
  ...
  const handleSave = async () => {
    ...
    if (image?.base64) {
      const fileName = `${Date.now()}_kariakoo.jpg`;
    ...
    let error;
    if (editingId) {
      const res = await supabase.from('kariakoo_items').update(payload).eq('id', editingId);
      error = res.error;
    } else {
      const res = await supabase.from('kariakoo_items').insert(payload);
      error = res.error;
    }
  ...
  return (
    <SafeAreaView style={[styles.safeArea, { backgroundColor: colors.background }]} edges={['top']}>
      <View style={styles.header}>
        ...
        <ThemedText style={{ fontSize: 20, fontWeight: '700', color: colors.text }}>Manage Kariakoo</ThemedText>
